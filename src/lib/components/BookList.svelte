...
<div class="min-h-screen bg-gray-50 px-4 py-8 max-w-7xl mx-auto">
	<!-- 検索とフィルターセクション -->
		...
		{:else}
			<div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				{#each $filteredBooks as book (book.id)}
					<div
						class="bg-white rounded-xl shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md p-5"
					>
						<div class="flex">
							{#if book.imageLinks?.thumbnail}
								...
								/>
							{:else}
								<div class="w-16 h-24 bg-gray-100 rounded-lg flex items-center justify-center mr-4">
